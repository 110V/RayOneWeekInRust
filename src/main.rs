...
fn ray_color(r:&Ray)->Color{
    ...
    let sphere_center = Point3::new(0.0,0.0,-2.0);
    if let Some(intersect) = ray_sphere(&r,0.5,sphere_center){
        let normal =  (sphere_center - intersect).to_unit();
        return Color::new(1.0,0.0,0.0);
    }
    (1.0-t)*Color::new(1.0,1.0,1.0) + t*Color::new(0.5,0.7,1.0)
}

fn ray_sphere(ray:&Ray,radius:f32,pos:Point3)->Option<Vec3>{
    let radius_squared = radius*radius;
    let new_pos = ray.origin + pos;
    if let Some(t) =  quadratic_equation(ray.dir.length_squared(), (2.0*new_pos*ray.dir).sum(), new_pos.length_squared()-radius_squared){
        let intersect = ray.at(t[0]);
...
fn main() {
    ...
    for y in range(0, IMG_HEIGHT){
        for x in range(0,IMG_WIDTH){
            let img_x = IMG_WIDTH-x-1;
            let img_y = IMG_HEIGHT-y-1;
            ...
            set_color(img_x, img_y, color, &mut img_array);
        }
    }
